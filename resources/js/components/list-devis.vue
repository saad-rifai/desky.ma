            ...
            <tbody>
              <tr v-for="(val, index) in infos" :key="index">
                ...
                <td>
                  ...
                  <span v-if="val.status == 1" class="uk-label uk-label-warning">في انتضار التسليم</span>
                  <span v-if="val.status == 2" class="uk-label uk-label-info">تم التسليم</span>
                  <span v-if="val.status == 3" class="uk-label uk-label-warning">في انتضار الدفع</span>
                  <span v-if="val.status == 4" class="uk-label ">  تم الدفع</span>
                  <span v-if="val.status == 5" class="uk-label uk-label-success">مكتمل</span>
                  <span v-if="val.status == 6" class="uk-label uk-label-info">بدون رد</span>
                  <span v-if="val.status == 7" class="uk-label uk-label-danger">ملغي </span>
                </td>
                <td>
                  <a href="javascript:void(0)"><button @click="showdevis(val.OID)" class="act-btn-radio">
...
export default {
  ...
  methods: {
    ...
    nextpage: function (page) {
      $('#form-loading').css('display', 'block')
      if (page < this.pages) {
    const config = { headers:{apikey: '35O3VOQQJCE947HA55EGCD07VFT32XCPDPMZET5H'} }
