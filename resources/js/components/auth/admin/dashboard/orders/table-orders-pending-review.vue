<template>
    ...
    <div >
        <review-order-modal v-if="modalOpen" :orderinfos="OrderSelected"></review-order-modal>

        <h5 class="card-title">Orders Pending Review ({{OrdersNumber}}) <span class="icon-btn" @click="refresh"><i class="fas fa-sync"></i></span></h5>
            ...
            <tr v-for="(item, index) in OrdersData" :key="index">
                ...
                <td>
                    ...
                    <div class="row row-cols-2 align-items-center">
                        ...
                        <div class="col-auto">
                            <a href="#" data-placement="top" data-toggle="tooltip" title="User account preview">
                ...
                <td dir="auto" align="center" class="max-w-200">{{item.title}}</td>
...
export default {
    ...
    getData(){
            //this.openLoadingInDiv();
            axios.get('/admin/ajax/orders/pending/get?page='+this.current_page).then((response) => {
                this.serverResponse = response.data.data
                this.OrdersNumber = response.data.CountPendingOrders
