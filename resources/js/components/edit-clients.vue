...
<div id="loading" class="form-loading"></div>
    <div class="uk-card uk-card-default uk-card-body">
      ...
      <div class="uk-width-1-1@m">
        <h2 class="uk-cart-title uk-text-left">N° {{ this.cid }}</h2>
...
export default {
  ...
  methods: {
    ...
    getClientsInfos: function(){
        axios.post('/clients/'+this.cid+'/'+this.uid+'/edit/json').then((response) => {
            $('#form-loading').css('display', 'none')
            this.c_name = response.data[0].c_name;
            this.c_phone = response.data[0].c_phone;
            this.c_email = response.data[0].c_email;
            this.c_type = response.data[0].c_type;
            if(response.data[0].c_ice != null){
            this.c_ice = response.data[0].c_ice;
            ...
            if(response.data[0].country != null){
            this.country = response.data[0].country;
            for(var i = 0; this.Countries.length > i; i++){
                if(this.Countries[i].code == response.data[0].country){
    ...
    saveClient() {
      ...
      axios
        .post('/api/v1/user/desky/edit/Clients', data)
        .then((response) => {
            UIkit.notification({
