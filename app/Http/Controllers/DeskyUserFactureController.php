...
class DeskyUserFactureController extends Controller
{
    public function index(Request $request){
        if(isset($request->OID) && isset($request->UID) && $request->OID != "" && $request->UID !="" && is_numeric($request->OID) && is_numeric($request->UID)){
            $OID = $request->OID;
            $UID = $request->UID;
            $DevisInfos = Cache::remember('desky_user_facture'.Auth::user()->id.'oid_'.$OID, 60*24*2, function() use($OID, $UID){
            ...
            if($DevisInfos->count() > 0){
                if(isset($request->datajson)){
                    ...
                    return view('desky.panel.facture.edit-facture', ['OID' => strval($request->OID)]);
                }
            }else{
                return abort(404);
            }
    ...
    public function GetLastFactureNumber(){
        $stmt = DeskyUserFacture::where("email", Auth::user()->email)->max("OID");
    ...
    public function CreateFacture(Request $request)
    {
        ...
        if ($unlimited == true || $points > 0) {
                    ...
                    if($countclientsCount > 0){
                        ...
                        ->where(function($qe) use ($request){
                            ...
                            if($go == 1){
                                ...
                                if($stmt){
                                    return response()->json(['sucess' => 'تم انشاء الفاتورة'], 220);
                                }else{
                                    return response()->json(['error' => 'fx0234 تعذر انشاء الفاتورة يرجى اعادة المحاولة'], 500);

                                }


                            }else{
                                return response()->json(['error' => 'fx0240 تعذر انشاء حساب للعميل يرجى اعادة المحاولة'], 500);
                        ...
                        $stmt = desky_user_clients::create([
                            ...
                            if($stmt){
                                ...
                                if($stmt){
                                    ...
                                    return response()->json(['error' => 'تعذر انشاء الفاتورة يرجى اعادة المحاولة'], 500);
            ...
            return response()->json(['errors' => ['items' => ['رصيدك غير كاف لانشاء الفاتورة يرجى تجديد اشتراكك']]], 422);
        }
    }
    public function ShowFacture(Request $request)
    {
        if (isset($request->OID) && isset($request->UID) && $request->OID != "" && $request->UID != "") {
            if (is_numeric($request->OID) && is_numeric($request->UID)) {
                ...
                $OID = $request->OID;
                $FactureInfos = Cache::remember('desky_user_facture'.Auth::user()->id.'oid_'.$OID, 60*24*2, function() use($OID){
                    return DB::table('desky_user_factures')
                    ->join('desky_user_clients', 'desky_user_factures.CID', '=', 'desky_user_clients.CID')
                ...
                $count = $FactureInfos->count();
                if ($count > 0) {
                    foreach ($FactureInfos as $FactureInfo);
    ...
    public function ListFacture(Request $request)
    {
            ...
            if (isset($request->search) && $request->search == true) {
                ...
                ->paginate(10);
                $infos = DB::table('desky_user_factures')
                ->join('desky_user_clients', 'desky_user_factures.CID', '=', 'desky_user_clients.CID')
                ->select('desky_user_factures.*', 'desky_user_clients.c_name', 'desky_user_clients.c_email', 'desky_user_clients.c_phone')
                ...
                $stmt = DB::table('desky_user_factures')
                ->join('desky_user_clients', 'desky_user_factures.CID', '=', 'desky_user_clients.CID')
                ->select('desky_user_factures.*', 'desky_user_clients.c_name', 'desky_user_clients.c_email', 'desky_user_clients.c_phone')
                ->where('email', Auth::user()->email)
                ->orderBy('created_at', 'DESC')
    ...
    public function update(Request $request){
        ...
        if ($success == true) {
            $OID = $request->OID;
            $stmt = DeskyUserFacture::where("OID", $request->OID)->where("email", Auth::user()->email)->update([
                'items' => $request->items,
                'remise' => $request->remise,
                'p_method' => $request->p_method,
                'p_condition' => $request->p_condition,
            ...
            if($stmt){
             $cache =  Cache::forget('desky_user_facture'.Auth::user()->id.'oid_'.$OID);
             if($cache){
