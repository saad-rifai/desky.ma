...
class DeskyUserClientsController extends Controller
{
    ...
    public function ShowClient(Request $request){
        if(isset($request->CID) && isset($request->UID) && $request->CID != "" && $request->UID !="" && is_numeric($request->CID) && is_numeric($request->UID)){
            $CID = $request->CID;
            $UID = $request->UID;
            $Clients = Cache::remember('desky_user_clients'.Auth::user()->id.'CID_'.$CID, 60*24*2, function() use($CID, $UID){
        ...
        if(isset($request->token) && $request->token != ""){
            if(isset($request->CID) && $request->CID != "" && isset($request->notes) && $request->notes != ""){
                    ...
                    if( mb_strlen($notes, 'UTF-8') <= 1500){
                       ...
                       if($stmt){
                       Cache::forget('desky_user_clients'.Auth::user()->id.'CID_'.$CID);
    ...
    public function GetLastCIDNumber(){
        $stmt = desky_user_clients::where("from", Auth::user()->email)->max("CID");
        return response()->json($stmt);
    }
    public function CreerClients(Request $request){
        ...
        if($stmt){
            return response()->json(['success' => 'تم انشاء العميل بنجاح !'], 220);
        }else{
    ...
    public function EditClients(Request $request){
        ...
        $stmt = desky_user_clients::all()->where("from", Auth::user()->email)->where("CID", $request->CID);
        if($stmt->count() > 0){
            $stmt = desky_user_clients::where('from', Auth::user()->email)->where('CID', $request->CID)->update([
                'c_email' => $request->c_email,
                'c_phone' => $request->c_phone,
                'c_name' => $request->c_name,
                'c_type' => $request->c_type,
                ...
                if($stmt){
                    Cache::forget('desky_user_clients'.Auth::user()->id.'CID_'.$CID);
                    return response()->json(['success' => 'تم تحديث البيانات بنجاح !'], 200);
                }else{
                    return response()->json(['message' => 'فشل تحديث البيانات'], 500);

                }
        }else{
            return response()->json(['message' => 'لم يتم ايجاد العميل رقم '.$request->CID.''], 404);
        }
    }
