    ...
    protected function create(Request $request)
    {
        $this->validate(
            ...
            [
                ...
                'lname' => 'required|min:3|max:11|regex:/^[\p{L} ]+$/u',
                'email' =>
                    'required|string|email|regex:/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/ix|max:80|unique:mysql_neryou.users',
                'phonenumb' =>
                    'required|regex:/[0-9]{9}/|unique:mysql_neryou.users|max:10',
                'country' => 'required|max:20|regex:/^[\p{L} 0-9]+$/u',
        ...
        if ($resultcheck == false) {
            return response()->json(
                ['errors' => ['country' => ['يرجى تحديد بلد صحيح']]],
                422
            );
        }
        ...
        if (
            isset($request->user_type_987251504) &&
            $request->user_type_987251504 == $PermissionId
        ) {
            ...
            if (
                ...
                if ($stm) {
                    ...
                    if ($cart_id != '') {
                        ...
                        $check = MyCart::where('OID', $OID)
                            ->where('status', 0)
                            ->get(['OID']);
                        if (count($check) > 0) {
                            $stmt = MyCart::where('OID', $OID)
                                ->where('status', 0)
                                ->update([
                                    'UID' => Auth::user()->id,
                            ...
                            if ($stmt) {
                                ...
                                echo json_encode(array('token' => Session::getId(), 'ref' =>  '/pay/' .Auth::user()->id .'/' .$OID, 'status' => 'ok'));
                    ...
                    try {
                        Mail::to($request->email)->send(
                            new verfymail_desky($valueArray2)
                        );
                    ...
                    return response()->json(
                        [
                            'error' =>
                                'لقد حصل خطأ ما أثناء محاولة انشاء المستخدم يرجى اعادة المحاولة',
                        ],
                        422
                    );
                }
