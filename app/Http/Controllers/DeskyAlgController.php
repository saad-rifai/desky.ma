           ...
           ->where("email", Auth::user()->email)
            ...
            foreach($datas as $data){
                ...
                if($date_y == date("y")){
                   ...
                   $n_items =  count($items);
                   for($i=0; $n_items > $i; $i++){
                       $price_y += intval($items[$i]->price*$items[$i]->quantity);
                   }
                   ...
                   $n_items =  count($items);
                   for($i=0; $n_items > $i; $i++){
                       $price_u += intval($items[$i]->price*$items[$i]->quantity);
                   }
                ...
                if($date_m == date("m") && $data->status == 4 ||  $data->status == 5){
                   ...
                   $n_items =  count($items);
                   for($i=0; $n_items > $i; $i++){
                       $price_m += intval($items[$i]->price*$items[$i]->quantity);
                   }
            ...
            return response()->json(['TotalThisMonth' => $TotalThisMonth, 'TotalThisYear' => $TotalThisYear, 'TotalUnpaidThisMonth' => $TotalUnpaidThisMonth],200);
